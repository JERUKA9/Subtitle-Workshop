...
uses
  ...
  StdCtrls, Math, LazarusPackageIntf;
...
begin
  ...
  if FMax <> 0 then
    FFillRect.Left := (FPosition * 100 div FMax) * (Width-FButtonSize.X) div 100
  else
    FFillRect.Left := 0;
...
procedure TUWSeekBar.SetMax(const AMax: Int64);
begin
  if FMax <> AMax then
  begin
    FMax := AMax;
...
procedure TUWSeekBar.SetPosition(const APosition: Int64);
begin
...
procedure TUWNumberBox.KeyDown(var Key: Word; Shift: TShiftState);
begin
  case Key of
     VK_UP   : DoValueUp;
     VK_DOWN : DoValueDown;
...
begin
  RegisterComponents('URUWorks', [TUWTimeEdit, TUWTickTime, TUWSeekBar,
    TUWLayout, TUWCPSBar, TUWNumberBox]);
end;
