...
uses SysUtils, StrUtils, Classes, Math, LazUTF8;
...
function SetDelay(const Time, Delay: Integer): Integer;                         // positive or negative, time or frames
...
function AutomaticDurations(const Text: String; const Duration, msPerChar, msPerWord, msPerLine: Integer; const Mode: TAutomaticDurationMode): Integer; overload; // calculate the duration of subtitles using a simple formula
function AutomaticDurations(const Text: String; const Duration, CPS: Integer; const Mode: TAutomaticDurationMode): Integer; overload;
...
function SetDelay(const Time, Delay: Integer): Integer;
begin
...
function AutomaticDurations(const Text: String; const Duration, msPerChar,
  msPerWord, msPerLine: Integer; const Mode: TAutomaticDurationMode): Integer;
var
  pc, pw,
  pl, nd: Integer;
begin
  ...
  pc := UTF8Length(Text) - StringCount(' ', Text) - StringCount(sLineBreak, Text);
  pw := WordCount(Text);
  pl := LineCount(Text);
...
function AutomaticDurations(const Text: String; const Duration, CPS: Integer;
  const Mode: TAutomaticDurationMode): Integer;
var
  nd: Integer;
begin
  Result := Duration;

  nd := Math.Ceil((UTF8Length(ReplaceString(Text, sLineBreak, '')) * 1000) / CPS);
