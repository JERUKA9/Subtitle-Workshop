  ...
  for i := 0 to FOCRList.Count-1 do
    ...
    begin
      if FOCRList[i]^.UseRE and (FOCRList[i]^.Method = rmNone) then
        Result := ReplaceRegExpr(FOCRList[i]^.Find, Result, FOCRList[i]^.ReplaceBy, False)
      else
        case FOCRList[i]^.Method of
          ...
          rmBegin: if AnsiStartsText(FOCRList[i]^.Find, Result) then
                   begin
                     Delete(Result, 1, Length(FOCRList[i]^.Find));
                     Result := FOCRList[i]^.ReplaceBy + Result;
                   end;
          rmEnd  : if AnsiEndsText(FOCRList[i]^.Find, Result) then
